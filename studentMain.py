...
def estimate_next_pos(measurement, OTHER = None):
    ...
    x,y = measurement
    if OTHER is None:
        OTHER = [x,y,0,0,0] #prev poz, theta, d,alfa
        theta, d,alfa = OTHER[2:]
    else:
        x0, y0, theta0, d0, alfa0 = OTHER
        dy=y-y0
        dx=x-x0
        koef = 0.6
        theta = atan2(dy, dx)
        d=sqrt(dy**2+dx**2)*koef+d0*(1-koef)
        alfa = (theta - theta0)%(2*pi)*koef+alfa0*(1-koef)
    xy_estimate = [x+d*cos(theta+alfa),y+d*sin(theta+alfa)]
    OTHER = [x,y,theta,d,alfa]
    print(measurement, xy_estimate, OTHER)
